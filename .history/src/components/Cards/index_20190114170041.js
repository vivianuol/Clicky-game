...
class Cards extends React.Component {
    ...
    handleOnClick = charName => {
        console.log(charName + "onclick!");
        let newPosts = this.shuffle(this.state.arr);
        this.setState({arr: newPosts})
    }

        render() {
            return (
                <div className="cards-layout">
                  {this.state.arr.map((item, i) => <Cartoon imageLoc={item.imageLoc} charName={item.charName} key={i} handleClick={this.handleOnClick(item.charName)} />)}
                </div>
            )
        }
    }
